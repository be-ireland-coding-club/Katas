export default class Yatzy {
  ...
  chance(): number {
    return this.dice.reduce((accumulator, current) => accumulator + current, 0)
  }

  yatzy(): number {
    return this.dice.every(e => e === this.dice[0]) ? 50 : 0;
  }

  ones(): number {
    return this.sameNumber(1);
  }

  twos(): number {
    return this.sameNumber(2);
  }

  threes(): number {
    return this.sameNumber(3);
  }

  fours(): number {
    return this.sameNumber(4);
  }

  fives(): number {
    return this.sameNumber(5);
  }
 
  sixes(): number {
    return this.sameNumber(6);
  }

  sameNumber(num: number): number {
    return this.dice.filter(d => d === num).length * num;
  }
